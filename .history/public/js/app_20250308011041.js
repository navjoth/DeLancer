...
const setReputationBtn = document.createElement('button');
setReputationBtn.id = 'setReputationBtn';
setReputationBtn.textContent = 'Set Freelancer Reputation';
document.querySelector('body').appendChild(setReputationBtn);
...
async function applyForProject() {
  ...
  try {
      ...
      const receipt = await contract.methods.applyForProject(projectId, bidAmount).send({ from: userAccount, gas: 3000000 });
      console.log('Applied for Project:', receipt);
...
async function setFreelancerReputation() {
  const freelancerAddress = prompt('Enter the freelancer address:');
  const reputation = prompt('Enter the reputation score:');
  if (!freelancerAddress || !reputation || isNaN(reputation) || parseInt(reputation) < 0) {
      alert('Invalid input. Please enter a valid address and reputation score.');
      return;
  }

  try {
      const receipt = await contract.methods.setFreelancerReputation(freelancerAddress, reputation).send({ from: userAccount });
      console.log('Reputation set:', receipt);
      alert(`Reputation set for ${freelancerAddress}: ${reputation}`);
  } catch (error) {
      console.error('Error setting reputation:', error);
      alert('Failed to set reputation. Check console for details.');
  }
}
  ...
  try {
      ...
      for (let i = 1; i <= projectCount; i++) {
          ...
          row.innerHTML = `
              ...
              <td>${project.bidCount || '0'}</td> <!-- Show number of bids -->
          `;
...
setReputationBtn.addEventListener('click', setFreelancerReputation);
