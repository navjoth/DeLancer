...
async function applyForProject() {
  ...
  try {
      ...
      console.log('Applied successfully for Project ID:', projectId);
...
async function depositFunds() {
  ...
  try {
      ...
      if (project.bidAmount == 0) {
          alert('No bid amount set. Assign a freelancer first.');
          return;
      }

      await contract.methods.depositFunds(projectId).send({ 
          from: userAccount, 
...
async function fetchFreelancers() {
  try {
      const projectCount = await contract.methods.projectCount().call();
      let freelancersSet = new Set();
      for (let i = 1; i <= projectCount; i++) {
          const bidders = await contract.methods.getProjectBidders(i).call();
          bidders.forEach(freelancer => {
              if (freelancer !== '0x0000000000000000000000000000000000000000') {
                  freelancersSet.add(freelancer);
              }
          });
      }

      const freelancers = Array.from(freelancersSet);
      freelancerTable.innerHTML = '<h3>Freelancer List</h3>';
      if (freelancers.length === 0) {
          freelancerTable.innerHTML += '<p>No freelancers found.</p>';
          return;
      }
      ...
      freelancerTable.appendChild(table);
  } catch (error) {
