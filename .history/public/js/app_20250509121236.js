...
async function createProject() {
  ...
  // Use a Promise to handle file selection or cancellation
  const fileSelectionPromise = new Promise((resolve) => {
    ...
    // Fallback for cancellation (not all browsers support 'cancel' event)
    fileInput.addEventListener('click', () => {
      // If the file dialog is canceled, the onchange won't fire, so we resolve after a timeout
      setTimeout(() => {
        if (!fileInput.files.length) {
          resolve(); // Treat as cancellation
        }
      }, 10000); // 10 seconds timeout for user to select a file
    });
  });

  try {
    // Attempt to open the file browser
    fileInput.click();
  } catch (error) {
    console.error('Error opening file browser:', error);
    alert('Failed to open file browser. Proceeding without file upload.');
...
document.getElementById('createProjectBtn').addEventListener('click', createProject);S
