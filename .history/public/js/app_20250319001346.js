...
async function fetchProjects() {
  try {
    ...
    for (let i = 1; i <= projectCount; i++) {
      ...
      let applicantsHtml = 'No applicants yet'; // Default value
      try {
        console.log(`Attempting to fetch feeProposals for project ${i}...`);
        const proposals = await contract.methods.feeProposals(i).call();
        console.log(`Raw proposals data for project ${i}:`, proposals);
        ...
        if (Array.isArray(proposals) && proposals.length > 0) {
          ...
          console.log(`No proposals or invalid data for project ${i}`);
        }
      } catch (proposalError) {
