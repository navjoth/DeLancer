...
contract HireChain {
    ...
    struct Project {
        ...
        bool completed;
        bool isPaid;
        Bid[] bids; // Array of bids from freelancers (defaults to empty in storage)
    }
    ...
    function createProject(string memory _name, string memory _description) public {
        ...
        projects[projectCount] = Project({
            ...
            completed: false,
            isPaid: false,
            bids: new BidExplicitly initializing an empty array
        });
    ...
    function getFreelancers() public view returns (address[] memory) {
        ...
        for (uint256 i = 1; i <= projectCount; i++) {
            totalBidders += projects[i].bids.length;
        }

        address[] memory allFreelancers = new address[](totalBidders);
        uint256 index = 0;
