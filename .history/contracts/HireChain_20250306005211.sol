...
contract HireChain {
    ...
    struct Project {
        ...
        bool completed;
        bool isPaid;
        mapping(address => Bid) bids; // Mapping of freelancer addresses to their bids
    ...
    function applyForProject(uint256 _projectId, uint256 _bidAmount) public {
        ...
        project.bids[msg.sender] = Bid(_bidAmount, freelancerRep);
        project.bidCount++;
    ...
    function assignFreelancer(uint256 _projectId) public {
        ...
        // Collect all bidder addresses (since mappings can't be iterated directly)
        for (uint256 i = 1; i <= projectCount; i++) {
            if (projects[i].bids[msg.sender].amount > 0) { // Check if a bid exists
    ...
    function getFreelancers() public view returns (address[] memory) {
        ...
        uint256 index = 0;
        for (uint256 i = 1; i <= projectCount; i++) {
            if (projects[i].bids[msg.sender].amount > 0) { // Check if freelancer has bid
